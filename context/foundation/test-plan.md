...
>
> Previously: 2026-08-03 (§3 Phase 2 marked complete — implemented and
...
| e2e | Playwright 1.x (checked: 2026-08-03), bootstrapped via `/10x-e2e` (standalone) | `tests/e2e/` | Phase 2's own risk (#1, account isolation) was provable at integration level for far less cost — no e2e added for it, per research. A standalone golden-path test now covers the PRD's primary flow instead: `tests/e2e/seed.spec.ts` — signup → sign-in → training-profile save → persists across navigation. Runs only against a local Supabase stack (`tests/e2e/global-setup.ts` refuses to run against the hosted project) |
...
| typecheck (`astro check`) | CI (added §3 Phase 4) | required | type drift the type-aware lint rules don't reject — notably a renamed/removed field in a third-party type, which Vitest cannot see (it strips types without checking them) and the build does not either. Corrected 2026-08-03: pre-commit runs only `eslint --fix` + `prettier`, so this gate is CI-side, not local |
